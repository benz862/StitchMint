...
export function CropTextLiveOverlay({
  ...
  /**
   * Reusable offscreen canvas for measureText calls. Stored in a ref so the same canvas/context is
   * reused across re-renders, mirroring how the server's drawStraightAtAnchor uses one node-canvas
   * context. Created lazily on the first effect run (browser-only) so SSR is unaffected.
   */
  const measureCanvasRef = useRef<HTMLCanvasElement | null>(null);

  /**
   * Live-fitted px size: identical pipeline to the server's drawStraightAtAnchor, so what the editor
   * shows is what gets rasterized into composition.png. Recomputes whenever the text, frame size,
   * anchor, or typography changes.
   */
  ...
  return (
    <div
      ...
      style={{
        ...
        lineHeight: 1.28,
        /**
         * `pre` (not `pre-wrap`) honors user-entered newlines but never auto-wraps on whitespace.
         * The shared computeOverlayFontSizePx already shrinks the font so a single-line title fits
         * the crop width — so "Glenn Donnelly" stays on one line at whatever size fits, instead of
         * breaking at the space between Glenn and Donnelly.
         */
        whiteSpace: "pre",
      }}
      {...dragHandlers}
